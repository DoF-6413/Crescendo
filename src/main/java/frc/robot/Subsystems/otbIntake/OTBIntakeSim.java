...
public class OTBIntakeSim implements OTBIntakeIO {
    private FlywheelSim intakeMotorSim;
    private double OTBIntakeAppliedVolts = 0.0;

    public OTBIntakeSim() {
        intakeMotorSim = new FlywheelSim(DCMotor.getNEO(1), Constants.OTBIntakeConstants.GEAR_RATIO,
            Constants.OTBIntakeConstants.OTB_MOMENT_OF_INERTIA_KGMETERSSQUARED);
    }

    @Override
    public void updateInputs(OTBIntakeIOInputs inputs) {
        inputs.OTBIntakepositionrad += intakeMotorSim.getAngularVelocityRadPerSec() * Constants.RobotStateConstants.LOOP_PERIODIC_SEC;
