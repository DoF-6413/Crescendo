 ...
 */
  ...
  public void updateInputs(ActuatorIOInputs inputs) {
    inputs.actuatorPositionRad = Units.rotationsToRadians(actuatorEncoder.getPosition())
        / ActuatorConstants.GEAR_RATIO;
    inputs.actuatorPositionDeg = Units.rotationsToDegrees(actuatorEncoder.getPosition())
        / ActuatorConstants.GEAR_RATIO;
    inputs.actuatorVelocityRadPerSec = Units.rotationsPerMinuteToRadiansPerSecond(actuatorEncoder.getVelocity())
        / ActuatorConstants.GEAR_RATIO;
    inputs.actuatorAppliedVolts = actuatorMotor.getAppliedOutput() * actuatorMotor.getBusVoltage();
    inputs.actuatorCurrentAmps = new double[] {
        actuatorMotor.getOutputCurrent()};
    inputs.actuatorTempCelsius = new double[] {
